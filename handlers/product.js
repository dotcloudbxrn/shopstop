...
module.exports = (req, res) => {
  ...
  if(req.pathname === '/product/add' && req.method === 'GET') {
    ...
    form.on('part', (part) => {
      if(part.filename) {
        ...
        part.on('end', () => {
          ...
          let frontEndPath = '/content/images/' + fileName + '.jpg';
          let saveFilePath = path.normalize(path.join(__dirname,'../content/images/' + fileName +'.jpg'))

          product.image = saveFilePath;
          fs.writeFile(
            `${saveFilePath}`, dataString, {encoding: 'ascii'}, (err) => {
              if(err) {
                console.log(err)
                return
              }
            })
          product.image = frontEndPath;
        })
      } else {
    ...
    res.write('DAMMIT! Resource not found');
    res.end();
  }
}